  ...
  for(seedInUse in seedsInUse){
    ...
    for(extrAttrToolName in otherExtrAttrToolNames){
      SynSigEval::SummarizeSigOneExtrAttrSubdir(
        run.dir = paste0(datasetName,"/sp.sp/ExtrAttrMMLDATest/",extrAttrToolName,
...
for(datasetName in datasetNames){
  ...
  for(extrAttrToolName in otherExtrAttrToolNames){
    SynSigEval::SummarizeMultiRuns(
      ...
      tool.dir = paste0(datasetName,"/sp.sp/ExtrAttrMMLDATest/",extrAttrToolName,
                          ".results/"),
...
for(slope in slopes){
  for(Rsq in Rsqs){
    ...
    SynSigEval::SummarizeMultiToolsOneDataset(
      third.level.dir = paste0(datasetName,"/sp.sp/ExtrAttrMMLDATest/"),
...
for(toolName in otherExtrAttrToolNames){
  SummarizeOneToolMultiDatasets(
    ...
    datasetSubGroupName = "SBS1:SBS5 mutation count ratio",
    toolName = toolName,
    tool.dirname = paste0("sp.sp/ExtrAttrMMLDATest/",toolName,".results/"),
    out.dir = paste0("FinalToolWiseSummary/ExtrAttrMMLDATest/",toolName,"/"),
    overwrite = T)
}
...
FinalExtrAttr <- SummarizeMultiToolsMultiDatasets(
  dataset.dirs = datasetNames,
  second.third.level.dirname = "sp.sp/ExtrAttrMMLDATest/",
  out.dir = "./FinalExtrAttrMMLDATestSummary", overwrite = T)
