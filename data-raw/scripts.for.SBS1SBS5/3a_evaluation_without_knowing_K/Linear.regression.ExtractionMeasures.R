...
for(toolName in c(extrAttrToolNames,toolNameWFixedSeed,toolNameWOSeed)){
  ...
  ForRegression[[toolName]]$TPR <-  OneToolSummary$TPR %>%
    dplyr::select(seed, value, datasetGroup, datasetSubGroup)

  ForRegression[[toolName]]$FDR <-  OneToolSummary$FDR %>%
    dplyr::select(seed, value, datasetGroup, datasetSubGroup)

  ForRegression[[toolName]]$cosSimSBS1 <-  OneToolSummary$cosSim$SBS1 %>%
    dplyr::select(seed, value, datasetGroup, datasetSubGroup)

  ForRegression[[toolName]]$cosSimSBS5 <- OneToolSummary$cosSim$SBS5 %>%
    dplyr::select(seed, value, datasetGroup, datasetSubGroup)

  indexes <- c("TPR","FDR","cosSimSBS1","cosSimSBS5")
...
for(toolName in c(extrAttrToolNames,toolNameWFixedSeed,toolNameWOSeed)){
  ...
  for(index in indexes)  {
    ...
    pdf(paste0("../../TrendDiag/ExtrAttr/",toolName,"/",index,"AgainstRsq.pdf"))
...
for(toolName in c(extrAttrToolNames,toolNameWFixedSeed,toolNameWOSeed)){
  for(index in indexes){
    ...
    pdf(paste0("../../TrendDiag/ExtrAttr/",toolName,"/",index,"AgainstSBS1SBS5ratio.pdf"))
...
save.image("../../TrendDiag/ExtrAttr/Linear.regression.ExtractionMeasures.Rdata")
