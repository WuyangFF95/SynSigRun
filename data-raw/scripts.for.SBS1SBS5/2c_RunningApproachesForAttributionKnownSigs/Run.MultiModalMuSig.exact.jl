...
for seedInUse in seedsInUse
	for datasetName in datasetNames
		snv_counts = CSV.read(datasetName*"/sp.sp/ExtrAttrExact/MultiModalMuSig.LDA.results/seed."*
		...
		CSV.write(datasetName*"/sp.sp/ExtrAttrExact/MultiModalMuSig.LDA.results/seed."*
...
for seedInUse in seedsInUse
	for datasetName in datasetNames
		...
		snv_counts = CSV.read(datasetName*"/sp.sp/ExtrAttrExact/MultiModalMuSig.CTM.results/seed."*
		string(seedInUse)*"/ground.truth.syn.catalog.tsv", delim='\t');
		...
		# You can't change row names by subsetting!
		CSV.write(datasetName*"/sp.sp/ExtrAttrExact/MultiModalMuSig.CTM.results/seed."*
		string(seedInUse)*"/extracted.signatures.tsv",
		snv_signatures,
		delim = "\t");
