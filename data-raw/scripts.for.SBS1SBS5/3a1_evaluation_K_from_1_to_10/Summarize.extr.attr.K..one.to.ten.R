...
RBasedExtrAttrToolNames <- c("hdp","sigfit.EMu",
                             "sigfit.NMF","signeR")
  ...
  for(seedInUse in seedsInUse){
    ...
    for(extrAttrToolName in RBasedExtrAttrToolNames){
      SynSigEval::SummarizeSigOneExtrAttrSubdir(
        run.dir = paste0(datasetName,"/sp.sp/ExtrAttrFromOne/",extrAttrToolName,
...
for(toolName in c(
  RBasedExtrAttrToolNames,otherExtrAttrToolNames,"EMu")){
...
FinalExtrAttrFromOne <- SummarizeMultiToolsMultiDatasets(
  dataset.dirs = datasetNames,
