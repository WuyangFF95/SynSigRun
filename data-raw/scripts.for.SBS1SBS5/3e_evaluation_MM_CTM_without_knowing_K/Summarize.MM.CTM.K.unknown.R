  ...
  for(seedInUse in seedsInUse){
    ...
    for(extrAttrToolName in otherExtrAttrToolNames){
      SynSigEval::SummarizeSigOneExtrAttrSubdir(
        run.dir = paste0(datasetName,"/sp.sp/ExtrAttrMMCTMTest/",extrAttrToolName,
...
for(datasetName in datasetNames){
  ...
  for(extrAttrToolName in otherExtrAttrToolNames){
    SynSigEval::SummarizeMultiRuns(
      ...
      tool.dir = paste0(datasetName,"/sp.sp/ExtrAttrMMCTMTest/",extrAttrToolName,
                          ".results/"),
...
for(slope in slopes){
  for(Rsq in Rsqs){
    ...
    SynSigEval::SummarizeMultiToolsOneDataset(
      third.level.dir = paste0(datasetName,"/sp.sp/ExtrAttrMMCTMTest/"),
...
for(toolName in otherExtrAttrToolNames){
  SummarizeOneToolMultiDatasets(
    ...
    datasetSubGroupName = "SBS1:SBS5 mutation count ratio",
    toolName = toolName,
    tool.dirname = paste0("sp.sp/ExtrAttrMMCTMTest/",toolName,".results/"),
    out.dir = paste0("FinalToolWiseSummary/ExtrAttrMMCTMTest/",toolName,"/"),
    overwrite = T)
}
...
FinalExtrAttr <- SummarizeMultiToolsMultiDatasets(
  dataset.dirs = datasetNames,
  second.third.level.dirname = "sp.sp/ExtrAttrMMCTMTest/",
  out.dir = "./FinalExtrAttrMMCTMTestSummary", overwrite = T)
